...
#dbconfig.php creates a database, database tables and prompts the user for an Admin password (this is the "see all" general admin)
	...
	else if($_SESSION['user'] != 'DBAdmin' && $_SESSION['user']['account_lid']!='Admin')
        {
	...
	else if (!empty($_POST['createtables']))
	{
	...
	else if (!empty($_POST['confirmdroptables']))
	{
		#echo " I am here";
		#$GLOBALS['dbsetup']->drop_tables();	
	}
	else if(!empty($_POST['createadmin']) &&!isset($_SESSION['adminset']))
	{
			
		create_admin_account($_POST['adminpassword'], $_POST['adminpasswordcheck'], $tpl);	
	}	
	else if(!empty($_POST['adminlogin']))
	{
        Header('location: home.php');
        exit;
    	}
	else if(!empty($_POST['updateadmin']))
	{
	...
	else 
	{
		...
		else 
		{
           ...
           #   	$tpl->set_var('db_action', '<input type="checkbox" name="register" checked>Register this S<sup>3</sup>DB at '.$GLOBALS['s3db_info']['deployment']['mothership']);
			...
			else if(!empty($_POST['resetadminpass']))
			{
		...
		else 
		{
			...
			else 
			{
				...
				if($dbdata['Errno']=='0')
				{
					...
					$user_info = array('account_id' => $account_id,
				      'account_lid' => 'Admin',
				      'account_uname' => 'Admin',
				      'account_group' => 'a');
