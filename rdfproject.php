...
$s3Elements = array('projects'=>array('dc:description'=>'project_description', 
									'dc:name'=>'project_name'),
				 'classes'=>array('dc:name'=>'entity'), 
				 'instances'=>array('dc:comment'=>'notes',
								'rdfs:class'=>'class_id'),
				 'rules'=>array( 'dc:comment'=>'notes', 
								'rdf:subject'=>'class_id',
								'rdf:predicate'=>'verb',
								'rdf:object'=>'object',
								),
				 'statements'=>array('rdf:subject'=>'instance_id',
...
#grab all the coreelements from the project, or if no project, grab all the coreelements where user has permissions
	...
	if($element=='class') $iid='0';
	elseif($element=='instance') $iid='1';
	...
	foreach($D[$element] as $n3statement)
		...
		for($i=0;$i<=count($prefixRef);$i++)
			...
			###THERE IS A BUG HERE WHEN OBJECT OF RULE IS CLASS, IT IS NOT PASSING CLASS_ID
...
if($_REQUEST['download']=='no')
	...
	##download the file
       ...
       #header("Content-Type: application/download");
		...
		#echo $filecontents = fread(fopen($filename, 'r'), filesize($filename));
