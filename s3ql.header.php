<?php
	...
	if($timer) { $timer->setMarker('ini set'); }
	...
	if($timer) $timer->setMarker('1/2 functions added');
	...
	if($timer) $timer->setMarker('All functions added');
	...
	if (empty($key[0])) {
		...
		if($_SESSION['db']!='') {
			...
			if(!ereg('(S3QL|s3ql|URI|rdf|rdfimport|mapproject|resource|project|item|rule|ruleinspector|s3dbfile|sparql|msredirect).php$',$_SERVER['PHP_SELF'])) {
	...
	function authenticate_OLD($key, $url) {
		if($key!='') {
			...
			if($key_valid) {
				...
				if($url=='') {
					...
					return 1;
					exit;
				} else {  	#URL contains info on user in the last part of the path. (for example: URL=https://ibl.mdanderson.org/s3db/U4)
		...
		} elseif ($key=='') {		#if there is a session, no need to authenticate the user again
			if ($_SESSION['db']!='') {
				$db = $_SESSION['db'];
				$user_id = $_SESSION['user']['account_id'];
			} elseif(in_array('key', array_keys($_REQUEST))) {		#the url seems prepared to take in a key, but it is empty
				echo '<S3QL>';
				echo '<error>0</error>';
