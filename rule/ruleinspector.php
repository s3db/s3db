				...
				<tr>
					<td>
						<table class="insidecontents" width="100%" align="center">
							<tr bgcolor="#99CCFF">
								<td colspan="3" align="center">Rule Inspector</td>
							</tr>
							<tr>
								<td></td>
								<td>Available Rules: '.count($rules).'</td>
								<td align="right">
									Number of Results Per Page
									<select name="num_per_page" onChange="window.location=this.options[this.selectedIndex].value">';
			foreach($perPage as $n) {
				echo '
										<option value="'.$thisScript.'&num_per_page='.$n.'" '.$select[$n].'>'.$n.'</option>';
			}
...
<?php
		echo '
	<form method="POST" name="insertAcl" action="ruleinspector.php?key='.$key.'&project_id='.$project_id.'&class_id='.$class_id.'" autocomplete="on">
		<table class="edit_rule" width="100%"  align="center" border="0">
			<tr>
				<td class="message" colspan="9"><br />'.$action_message.'</td>
			</tr>
			<tr bgcolor="#80BBFF">
				<td colspan="9" align="center">'.$edit_message.'<input type="hidden" name="rule_id" value="'.$rule_id.'"><input type="hidden" name="project_id" value="'.$project_id.'"></td>
			</tr>
		...
		# echo '<td  width="10%">'.$subject_input.'</td>'; 
		# echo '<td  width="10%">'.$verb_input.'<a href="'.$action['peekverb'].'" target="_blank"><I>Select an instance</I></a></td>';
		echo '
				<td width="10%">'.$class_select.'</td>
				<td width="10%" id="verb_holder">'.$verb_select.'</td>
				<td width="10%" id="object_holder">'.$object_select.'</td>
				...
				<td width="10%" align="center"><input type="submit" name="'.$action_name.'" value="'.$action_value.'"></td>
			</tr>
			<tr>
