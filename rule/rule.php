<?php
	...
	} else {		//if the buttons aren't clicked each reload of the page should run the following code
...
?>	
<form name="queryresource" method="POST" action="<?php echo $action['queryresult']; ?>&main_resID=<?php echo $_REQUEST['main_resID']; ?>&main_rule=<?php echo $_REQUEST['main_rule']; ?>" autocomplete="on">
	<table class="resource_list" width="100%" align="center" border="0">
		<tr>
			<td>
				<table class="query_resource" width="100%" border="0">
					<tr>
						<td class="nav_menu" colspan="9"></td>
					</tr>
					<tr>
						<td class="nav_menu" colspan="9"><hr size="2" align="center" color="dodgerblue"></hr></td>
					</tr>
					<tr>
						<td class="message" colspan="9"></td>
					</tr>
					<tr>
						<td class="nav_menu" colspan="9" align="left">
							<input type="submit" name="search" value="Search <?php echo $class_info['entity'] ?>">&nbsp;&nbsp;&nbsp;
							<input type="button" name="listall" value="List all <?php echo $entity; ?>" onClick="window.location='<?php echo $action['querypage']; ?>'&listall=yes&main_resID=<?php echo $_REQUEST['main_resID']; ?>&main_rule=<?php echo $_REQUEST['main_rule']; ?>'">&nbsp;&nbsp;&nbsp;
							<input type="button" name="clearquery" value="Clear Query" onClick="window.location='<?php echo $action['querypage']; ?>'"><br /><br />
						</td>
					</tr>
					<tr>
						<td colspan="9">Note: Regular Expression can be used in the specification of query. See examples <a href="#" onClick="window.open('../docs/regularExpressionExample.html', 'help', 'width=450, height=600, location=no, titlebar=no, scrollbars=yes, resizable=yes')" title="Examples of Regular Expression">here</a></td>
					</tr>
					<tr>
						<td colspan="6">Available Rules: <?php echo count($rules)?></td>
						<td align="right">
...
<?php
	...
	foreach($num_per_page as $num) {
		echo '
								<option value="'.$action['querypage'].'&num_per_page='.$num.'" '.$selected[$num].'>'.$num.'</option>';
	}
?>
                         	</select>
                         </td>
