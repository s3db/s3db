<?php
	...
	if($_REQUEST['url']!='' && $_REQUEST['publicKey']!='') {
		...
		echo formatReturn($GLOBALS['error_codes']['wrong_input'],'s3rl.php is a function to retrieve the url of a specific Did. For documentation and usage please refer to http://s3db.org/documentation', $format, '');
		exit;
	}
	...
	function registerURL($U) {		#this function call the db but it should NOT leave this one function
		extract($U);
		...
		if(!$Did && $case=='registernewURL') { $Did = s3id(); }
		...
		if($inputValid){
			switch ($case) {
				case 'registernewURL':
				{
					...
					$_REQUEST['description'] = is_base64_encoded_s($_REQUEST['description'])?base64_decode($_REQUEST['description']):$_REQUEST['description'];
					...
					if($db->Errno==0) {
						...
						$remoteUser = $url.((substr($url, strlen($url)-1,1)=='/')?'':'/').'U1';
						$user_id = '1';
						...
						if($msg['error_code']=='0' || $msg['error_code']=='4') {
							...
							$item_id = $msg['item_id'];
							$msg1=unserialize($done);$msg1=$msg[0];
							
							##prepare a file to save this info
							if(is_file(S3DB_SERVER_ROOT.'/tmp/mothership_info_file') && $_REQUEST['clean']=='mothership') {	
								unlink(S3DB_SERVER_ROOT.'/tmp/mothership_info_file');
							}
							if(is_file(S3DB_SERVER_ROOT.'/tmp/mothership_info_file')) {
								$mothership_info = unserialize(file_get_contents(S3DB_SERVER_ROOT.'/tmp/mothership_info_file'));
								$rules = $mothership_info['rules'];
							} else {
								...
								$mothership_info['rules'] = $rules;
								file_put_contents(S3DB_SERVER_ROOT.'/tmp/mothership_info_file', serialize($mothership_info));
							}
								...
								foreach ($rules as $key=>$rule_info) {
									if(in_array($rule_info['object'], array_keys($_REQUEST))) {
										...
										if($rule_info['object']=='keywords') {
											...
											$v=$_REQUEST[$rule_info['object']];
											if(is_base64_encoded_s($v)) { $v=base64_decode($v); }
											$s3ql['where']['value']=$v;
											$done = S3QLaction($s3ql);
										} elseif($rule_info['object']=='name') {
											$s3ql['where']['value']=urldecode($name);
											$done = S3QLaction($s3ql);
										} else {
											$v=urldecode($_REQUEST[$rule_info['object']]);
											$s3ql['where']['value']=$v;
											$done = S3QLaction($s3ql);
										}
									}
				...
				case 'updateURL':
				{
					...
					if($message!='') { #check if it matches whatever is in store for this Did
						...
						##detect which version was used to create this public key... it's sad but necessary due to non-backward compatible RSA_crypt :(
						if(ereg("BEGIN PUBLIC KEY", base64_decode($publicKey))) {
							set_include_path(get_include_path() . PATH_SEPARATOR . 'pearlib/phpseclib');
							include('pearlib/phpseclib/Crypt/RSA.php');
							$rsa = new Crypt_RSA();
							$rsa->setEncryptionMode(CRYPT_RSA_ENCRYPTION_OAEP);
							$publicKey = base64_decode($publicKey);
							$rsa->loadKey($publicKey);
							$encripted = $rsa->encrypt($message);
						} else {
							#encript it
	...
	function is_base64_encoded_s($data) {
