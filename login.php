<?php
	...
	$tpl->set_var('different_user_link','
		...
		$tpl->set_var('different_user_link','<td width="40%"><br /><br /></td>');
	}
	...
	if(!empty($authorities)) {
			$auth_login .= '
	<TD align="RIGHT" width="40%"><font color="#000000"><i>Authority</i>:&nbsp;</font></TD>
	<TD>
		<select name="login_authorities" id="authorities" style="width: 155px">
			<option value="" selected>S3DB</option>';
			foreach ($authorities as $auth_info) {
				#if($auth_info['True/False']=='T') $endorsed='(e)';
				if(eregi('^t', $auth_info['Endorsed'])) { 
					$endorsed='(e)'; 
				} else { 
					$endorsed='';
				}
				$auth_login .= '
			<option value="'.$auth_info['DisplayLabel'].'">'.$auth_info['DisplayLabel'].' '.$endorsed.'</option>';
			}
			$auth_login .= '
		</select>
	</TD>';
