 ...
 *
 *
 * @version  $Id: Model.php,v 1.48 2007/05/01 12:59:18 cweiske Exp $
...
class Model extends Object
{
    ...
    */
    function _parseSparqlQuery($query)
    {
	 ...
	 function s3db_parseSparqlQuery($query)
    {#Stays here until i figure out a way to call this function without messing up the original one
        if ($this->queryParser === null) {
            require_once RDFAPI_INCLUDE_DIR . 'sparql/SparqlParser.php';
            $this->queryParser = new SparqlParser();
        }
       #echo '<pre>';print_r($this); exit;
		$parsedQuery = $this->queryParser->s3db_parse($query);
		#echo '<pre>';print_r($parsedQuery);exit;
		return $parsedQuery;
    }//function _parseSparqlQuery($query)
	...
	{
		#echo '<pre>';print_r($this);exit;
		return ($this->queryParser->query->base);
	}
