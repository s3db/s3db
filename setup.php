<?php
    ...
    $tpl->set_file(array('config' => 'configure.tpl'));
    ...
    if($_POST['Save_configuration'] || $_POST['Create_configuration']) {
        ...
        } elseif($_POST['Create_configuration']) {
    ...
    function validate_inputs($inputs) {
        ...
        } elseif($inputs['site_config_admin_pass'] == '') {
            $return =  'site_config_admin_pass';	
        } elseif($inputs['db_type'] == '') {
            $return =  'Database Type';	
        } elseif($inputs['db_host'] == '') {
        	$return =  'Database Host';	
        } elseif($inputs['db_name'] == '') {
            $return =  'Database Name';	
        } elseif($inputs['db_user'] == '') {
    ...
    function createS3DBDatabase($inputs) {
        ...
        if($db_engine == 'mysql') {
            ...
            if(!$dbconn) {
                error_log("Failed to connect to database (" . mysqli_connect_errno() . "): " . mysqli_connect_error());
            ...
            $dbcsql .= "GRANT ALL PRIVILEGES ON {$db_engine}.* TO '$user'@'$hostname' IDENTIFIED BY '$pass';";
            if(mysqli_multi_query($dbconn,$dbcsql)) {
                //Intentionally not echoing anything
                //echo "Database $dbname created successfully.";
            } else {
                error_log("Error creating database: " . mysqli_error($dbconn));
                return false;
            }
            mysqli_close($dbconn);
            ...
            if (!$dbconn) {
                error_log('Could not connect: ' . pg_last_error());
                return (False);
            }
            $dbcsql = "CREATE DATABASE $dbname";
            $pg = pg_query ($dbconn, $dbcsql);
        }
        return True;
    }
    ...
    function send_public_key($U) {
        ...
        if(in_array('curl', get_loaded_extensions())) {
            ...
            $curl_connection = curl_init($url2register);
