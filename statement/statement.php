...
//        }
	...
	function render_substatements_without_grid($exist_stats, $acl) 
	{
		...
		foreach($exist_stats as $i => $value) 
		{
			...
			{
				if (is_numeric($exist_stats[$i]['value'])) $resource_id=$exist_stats[$i]['value'];			
				else
				$resource_id='';
				...
				{       
						...
						{
						$instances = Array('resource_id'=>$db->f('resource_id'),
											'notes'=>$db->f('notes'),);
						}
			...
			else if($exist_stats[$i]['file_name'] !='')
			{
