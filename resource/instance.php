...
$class_id = ($_REQUEST['class_id']!='')?$_REQUEST['class_id']:$instance_info['class_id'];

if($instance_id=='')
{
echo "Please provide an instance_id";
...
elseif($instance_info['add_data'])
	...
	#add the instance header and the insertall
	echo '<table width="100%">';
	echo '<tr><td class="nav_menu">';
	echo '<br /><br />[ <a href="#" onClick="window.location=\''.$action['instanceform'].'\'"> Insert All Page </a>]';
	...
	{$action = $GLOBALS['action']; #all the possible links were separated ina script that gets always included
		...
		if(is_array($stats_per_rule) && is_array($tRules))
		{
			...
			foreach($stats_per_rule as $rule_id=>$exist_stats)
			{
				...
				$stats .= sprintf("%s\n", '	</td></tr>');
				#$exist_stats = get_exist_stats($instance_info['resource_id'], $rule_id, $db);
				#$s3ql= compact('user_id', 'db');
				#$s3ql['from'] = 'statements';
				#$s3ql['where']['project_id'] = $project_id;
				#$s3ql['where']['resource_id'] = $instance_info['resource_id'];
				#$s3ql['where']['rule_id'] = $rule_id;

				#$exist_stats = S3QLaction($s3ql);
				
				#$exist_stats = include_statement_acl($exist_stats, $user_id, $db);
				
				#echo '<pre>';print_r($exist_stats);
				$stat ='';
        ...
        {$action=$GLOBALS['action'];
				...
				foreach($exist_stats as $i => $value)
                {
						...
						if($exist_stats[$i]['object_id']!='')
						{
							...
							#if the value is numeric and not a file, put a button
                        ...
                        else
						{
							...
							$account_id = $exist_stats[$i]['created_by'];

							if($exist_stats[$i]['file_name'] !='')
							   {
									$statement_id = $exist_stats[$i]['statement_id'];
									$project_id = $exist_stats[$i]['project_id'];
									$resource_id = $exist_stats[$i]['resource_id'];
									$rule_id = $exist_stats[$i]['rule_id'];
									$file_name = $exist_stats[$i]['file_name'];
									if($exist_stats[$i]['file_size']!='') 
									 {	$size = ceil($exist_stats[$i]['file_size']/1024);
										$file_size = ' ('.$size.' kb)';
									}
									$substats .= "<tr><td colspan=6><font color=dodgerblue size=3><b>File: <a href=".$action['download']."&resource_id=$resource_id&rule_id=$rule_id&statement_id=$statement_id>".$file_name.$file_size."<a/></b></font></td></tr>";
								}
