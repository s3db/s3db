...
echo "<tr bgcolor='#80BBFF'><td colspan='9' align='center'>Import ".$resource_info['entity']." from file</td></b></tr>";
echo "<td><BR></td><tr><td bgcolor='#FFFFCC'>1. Select file to import</td></tr>";
echo '<tr><td><form enctype="multipart/form-data" name="importform" action="'.$action['excelimport'].'" method="POST">';
echo '<input type="hidden" name="MAX_FILE_SIZE" value="2000000"><input name="import" type="file"><BR><BR>	<input type="submit" name="submitfile" value="Submit File"></td></tr>';
echo "<tr><td><br /></td></tr>";
echo "<tr><td><p><font color='#FF3300'>Please Note</font>: <ul>
			...
			$instance_info = URIinfo('I'.$cells['data'][$row]['UID'], $user_id, $key, $db);
				...
				for($col=2;$col<count($objects);$col++)
					{#read the existing statements
						...
						#echo $col.'<pre>before';print_r($cells['data'][$row]);
						$cells['data'][$row][$col] = array('newvalue' => $cells['data'][$row][$col]);
						
						if ($rule_id!='' && is_array($instance_info))
									
						#find statement info associated with a determinate rule_id and UID (still to solve the problem of multiple statements)
							{
		...
		for ($col=0;$col<count($cells['rules']);$col++) {
			...
			elseif ($col>=2) {
				...
				if($statement_info['value']=='' && $row_data[$col]['newvalue']=='')
					{$tablecells .= '';}#print nothing no a and no b
...
#echo $done;exit;
		...
		elseif($confirm=='insertstatement')	{ #CREATE statement
				...
				#echo 'rule<pre>';print_r($cells['rules'][$col]);
				#echo $col.'before<pre>';print_r($cells['data'][$row][$col]);
				
				if($cells['rules'][$col]['rule_info']['object_id']!='' && $_POST['selectstatement_'.$row.'_'.$col]!='')
				{
				#echo $row.'ola.'.$col.'.'.$cells['data'][$row][$col]['newvalue'];
