<?php
	...
	function safe_args($expected, $recieved, $line='??', $file='??') {
		...
		if(count($recieved) != 0) {
			...
			} else {		/* if used as standard function we loop thru and set by position */
			    for ($i = 0; $i < $num; $i++) {
					$types[$expected[$i]['name']] = $expected[$i]['type']; 
 				}
	...
	*/
	...
	function ExecMethod($method, $functionparams = '_UNDEF_', $loglevel = 3, $classparams = '_UNDEF_') {
		...
		} elseif($partscount >= 3) {		/* if the $method includes a parent class (multi-dimensional) then we have to work from it */
