...
switch ($code) {
	...
	case 'P':
		$ruleValid[$element]=array(
						'<error>'.$error_codes['something_missing'].'</error><message>Project Name cannot be empty</message>'=>!empty($inputs['project_name'])
						);
	break;
	case 'C':
		$ruleValid[$element]=array(
	...
	break;
	case 'I':
		$ruleValid[$element]=array('<error>'.$error_codes['nothing_to_change'].'</error><message>Nothing Was Changed</message>'=>!nothing_changed($inputs, $oldvalues),
...
#Does the permission on user comply with the model? Convert if not
	...
	if($isOwner)
	{	#can only give permission that he himself effecitvelly has. replace all numbers with nsy
