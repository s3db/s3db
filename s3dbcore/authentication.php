<?php
	...
	function login($username, $password, $authority=false) {
		...
		if($username!='' && $password!='') {
			...
			return (array(false, "Please provive username and password"));
			exit;
		}
	...
	function create_authentication_proj($db) {
		...
		$important_rules = array(
								...
								'Authorities'=>array(
											array('verb'=>'validatesThrough','object'=>'Protocols'), 
	...
	function check_user_management($C) {
		...
		foreach ($important_rules as $subName=>$subNameRule) {
			foreach ($subNameRule as $ind=>$rule_info) {
				...
				if($file_fill && eregi('serviceAccountPassword|serviceAccountUserName|Template',$rule_info['object'])) {
					$done=array();
					$s3ql=compact('user_id','db');
					$s3ql['insert']='user';
					$s3ql['where']['rule_id']=$user_proj[$rule_info['object']]['rule_id'];
	...
	function create_authentication_proj1($db) {
		...
		*/
		...
		$default_items = array(
							...
							'Authorities'=>array(
											'google'=>array('URI'=>'https://www.google.com/accounts/ClientLogin', 'Protocols'=>'http', 'DisplayLabel'=>'google')
										)
						);
		#and now, as a final obsessive step... try to create it again;
