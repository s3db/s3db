<?php
	...
	function s3db_display($datasource, $columns) {
		...
		if(is_array($datasource)) { 
			$totalElements = count($datasource); 
		} else {
			$totalElements = '1';
		}
	...
	function render_elements($datasource, $acl, $columns, $table, $new=0, $uid='', $how_many=50) {
		...
		if(in_array('Show', $columns)) {
			$dg->addColumn(new Structures_DataGrid_Column('Show<br><input type=button value="Check All" onClick="this.value=check()">', null, null, array('width'=>'5%', 'align'=>'left','valign'=>'top'), null, 'printShowMe()'));
		}
	...
	function printverbAndId($params) {
		...
		if($_SESSION['previous_verb'] =='') {
			...
			$_SESSION['current_color'] = '0';
		} elseif($_SESSION['previous_verb']!=$record['verb']) {
			$_SESSION['previous_verb'] = $record['verb'];
	...
	function printstatus($params) {
		...
		if($record['status']=='Denied')  {
			$record['status'] = '<font color=red>'.$record['status'].'</font>';
		}
	...
	function printActionLinkPermission($params) {
		...
		if($record['status']!='deleted') {
			...
			} else {		#If the entry is from ouside project to this project, you can give permissions or not
	...
	function printVerbStat($verb) {
		...
		} elseif($_SESSION['previous_verb']!=$verb) {
			$_SESSION['previous_verb'] = $verb;
			$_SESSION['current_color'] = intVal($_SESSION['current_color']) + 1;
		}
	...
	#TODO: BI find out where it is called from 
	...
	function printACL_options($params) {
		...
		if($new) {
			...
			$checked = array(
							'V'=>array(
									'0'=>(($view==''||$view=='0')?'checked':''),	
									'1'=>(($view=='1')?'checked':''), 
									'2'=>(($view=='2')?'checked':''),
									'm'=>(($view=='m')?'checked':'')
								),
							'C'=>array(
									'0'=>(($change==''||$change=='0')?'checked':''), 
									'1'=>(($change=='1')?'checked':''), 
									'2'=>(($change=='2')?'checked':''),
									'm'=>(($change=='m')?'checked':'')
								),
							'A'=>array(
									'0'=>(($add_data==''||$add_data=='0')?'checked':''),
	...
	function printOwner($params) {
		...
		if($record['owner']!='') {
			$owner = $record['owner'];
		} elseif($record['created_byID']!='') {
			$owner = $record['created_byID'];
		}
		return $owner;
	}
	...
	function printVerbinColor($params) {
		...
		if($_SESSION['previous_verb'] =='') {
			$_SESSION['previous_verb'] = $record['verb'];
			$_SESSION['current_color'] = '0';
		// } elseif(strcasecmp($_SESSION['previous_verb'], $record['verb']) != 0) {
	...
	function printeditlinkResource($params) {
		extract($params);
		if($record['owner'] == $_SESSION['user']['account_lid'] || get_project_owner() == $_SESSION['user']['acccount_id'] || get_resource_permissions($record['project_id']) >=6) {
	...
	function printDeleteLinkResource($params) {
		extract($params);
		if($record['owner'] == $_SESSION['user']['account_lid'] || get_project_owner() == $_SESSION['user']['acccount_id'] || get_resource_permissions($record['project_id']) == 5 || get_resource_permissions($record['project_id']) == 7) {
