...
function create_dictionary_project($d,$db,$user_id)
	...
	foreach ($Dict as $di=>$dict_info) {
			...
			foreach ($d['rules'] as $triple) {
				...
				else $s3ql['where']['object'] = $triple[2];
...
function insert_new_ns($params,$Dict,$db,$user_id)
{#params should be an array with keys qname and url
 $ns = $params['qname'];
 $url = $params['url'];
...
function insert_not($params,$Dict,$db,$user_id)
	...
	if ($msg['item_id']) {
		...
		$done = S3QLaction($s3ql);
		$msg=unserialize($done);$msg = $msg[0];
		if($msg['statement_id']) {$d['uid'] = $params['uid'];
		...
		foreach ($verbs as $p=>$verb) {
			...
			if($ns_id){
				...
				$done = S3QLaction($s3ql);
				$msg=unserialize($done);$msg = $msg[0];
				if($msg['statement_id']) $d['s3dbLinks|'.$verb.'|s3dbNamespaces'] = $ns_id;
				...
				$done = S3QLaction($s3ql);
				$msg=unserialize($done);$msg = $msg[0];
				if($msg['statement_id']) $d['s3dbLinks|'.$verb.'|s3dbNamespaces'] = $term;
			}
			else {
				$s3ql=compact('user_id','db');
				$s3ql['insert']='statement';
