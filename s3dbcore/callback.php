<?php
	...
	function grab_resource_id($statement) {		#call it statement but its the same for classes and instances
	...
	function uid($complete_uid) {		
		...
		if(!ereg('(.*)/(.*)', $complete_uid, $out)) {
			...
			if(!ereg('(.*)#(.*)$', $complete_uid, $out1)) {
				...
				$uid = $Did.'/'.$uid;
			} else {		#...except when MS is identified. In this case, Did is everything between cardinal and Uid :-)
				$MS = $out1[1];
