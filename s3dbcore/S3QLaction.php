		...
		if(count($core_score)=='1' || (count($core_score)=='2' && $info[$GLOBALS['Did']]!=''))
			...
			$children = array('deployment'=>array('project', 'user', 'group'),
				...
				foreach ($dep_resource[$child] as $ind=>$Dinfo) {
					...
					if(!is_array($orderedDependencies[letter($dep_key)]))  $orderedDependencies[letter($dep_key)]=array();
					...
					if(!empty($children[$child]))
						{
							...
							if(!is_array($dep_resource[$grandChild])) $dep_resource[$grandChild]=array();
							foreach ($tmp as $ggChild) {
							 $depdep_key = strtoupper(substr($grandChild, 0,1)).$ggChild[$GLOBALS['s3ids'][$grandChild]];
							 
							 array_push($dep_resource[$grandChild], $ggChild);	
							 $dependencies[$depdep_key] = $ggChild;
							 
							 if(!is_array($orderedDependencies[letter($depdep_key)]))  $orderedDependencies[letter($depdep_key)]=array();
...
function retrieveUIDInfo($Z)
{
	...
	if ($s3ql['where'][$id]!='') {
		...
		#try external URI (external) but only for uid that are not being inserted and there is not indication of literal data
			...
			//Added	270709 for remote deploymnet
			else {
