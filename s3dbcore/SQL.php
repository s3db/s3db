<?php
	...
	function get_rule_id_by_entity_id($resource_class_id, $project_id, $db) {
		global $regexp;
		#$db = $_SESSION['db'];
		if($resource_class_id!='') {		##Find the entity
...
//	}
	...
	function classID4entity($P) {
		...
		extract($P);
		$sql = "select resource_id from s3db_resource as c, s3db_rule as r where (r.project_id ".$regexp." '^".$project_id."$' or r.permission ~ '(^|_)".$project_id."_') and r.subject=c.entity and r.project_id=c.project_id and r.object='UID' and iid='0' and entity = '".$entity."'";
		$db->query($sql, __LINE__, __FILE__);
		if($db->next_record()) {
			return $db->f('resource_id');
		}
	...
	function classID4verb($P) {
		...
		#P needs $object, $project_id, $db
		extract($P);
		$sql = "select resource_id from s3db_resource as c, s3db_rule as r where (r.project_id ".$regexp." '^".$project_id."$' or r.permission ~ '(^|_)".$project_id."_') and r.subject=c.entity and r.project_id=c.project_id and r.object='UID' and iid='0' and entity = '".$verb."'";
		$db->query($sql, __LINE__, __FILE__);
		if($db->next_record()) {
			return $db->f('resource_id');
		}
	...
	function findNonSharedSubjectClass($C) {
		...
		while($db->next_record()) {
			$rule_classes[] = array(
								'classes'=>$db->f('resource_id'), #the deformed hybrid :-)
								'rules'=>$db->f('rule_id')
							);
		}
		return ($rule_classes);
		...
		for ($i=0; $i < $I; $i++) {		#remember we are starting in 0!!!
			if($stream=='upstream') {
				...
				if($strictsharedwith) {
					...
					$regexpA['uid'] = $GLOBALS['regexp'];
				}
				if($strictuid) {
					$regexpA['shared_with'] = '=';
				} else {
					$regexpA['shared_with'] = $GLOBALS['regexp'];
				}
			} else {
				$uid = 'shared_with';
				$shared_with = 'uid';
