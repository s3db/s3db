<?php
	...
	if($user_id!='') {
		...
		$s3Types = array(
					'deployment'=>array('user', 'group', 'project'),
					'group'=>array('user'),
		...
		switch($specified_id_code) {
			case 'D':
				...
				$objectPredicates = array(
										'rdfs:comment'=>($specified_id_info[$COREcomment[$specified_id_type]]!='')?'"'.$specified_id_info[$COREcomment[$specified_id_type]].'"':'',
			...
			case 'P':
				...
				$objectPredicates = array(
										'rdfs:comment'=>($specified_id_info[$COREcomment[$specified_id_type]]!='')?'"'.$specified_id_info['project_description'].'"':'',
										#'a'=> $N3coreNames[$specified_id_type],
										'rdfs:label'=>'"'.$specified_id_info[$CORElabel[$specified_id]].'"',
	...
	function addCollectionItemStats($user_id,$db, $class_id, $inputs) {
		...
		if(is_array($items) && !empty($items)) {
			foreach($items as $key=>$item_info) {
				...
				$objectPredicates = array_filter($objectPredicates);
				if(is_array($objectPredicates) && !empty($objectPredicates)) {		#Global "this is a resource" statement
	...
	function addRuleStats($user_id,$db, $url,$rule_id, $inputs) {
		...
		if(is_array($stats) && !empty($stats)) {
			foreach($stats as $key=>$stat_info) {
				...
				$objectPredicates=array(
									'rdf:subject'=>n3UID($COREletter[$subject_name].$stat_info[$subject_name]),
	...
	function getSubClassStats($uid, $subClass_info, $inData, $user_id, $db, $N3coreNames, $inputs, $metaData) {
		...
		switch($a_class_type) {
			...
			case 'item':
				$objectPredicates = array(
									'rdfs:label'=>'"'.$subClass_info['notes'].'"',
