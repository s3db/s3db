...
if ($_POST['submitschema'])
	...
	#Action to be taken when an N3 file is submitted
elseif ($_POST['submitN3'])
	{
...
<table class="insidecontents" width="80%" align="center">
	<tr bgcolor="#99CCFF">
		<td align="center">Import Project</td></tr>
		<td align="left"></td>
	<tr>
		<td>
		<?php
		echo '<form enctype="multipart/form-data" name="importschema" action="'.$action['listprojects'].'"" method="POST"><input type="hidden" name="MAX_FILE_SIZE" value="20000000">';
		
		echo '
		<input type="file" name="schema"><input type="submit" name="submitschema" value="Import from XML" '.$disabled.'> <font size="2">';
		?>
		(valid for XML files created by S3DB or complying with the <a href="../docs/s3db.xsd" target='_blank'> S3 schema </a>)</font>
		</form></td></tr><br><br>
		<td align="left"></td>
		<tr><td><form enctype="multipart/form-data" name="importN3" action="<?php echo $action['listprojects']; ?>" method="POST"><input type="hidden" name="MAX_FILE_SIZE" value="20000000"><input type="file" name="schema"><input type="submit" name="submitN3" value="Import/Update from N3/RDF"  <?php echo $disabled ?>><font size="2">(valid only for N3/RDF files annotated to <a href="../docs/s3dbCore.n3" target="_blank">S3DB core</a>). <br />Please note that RDF uploads through the interface only accepts small files (<10000 lines). For larger files, please use API function: rdfRestore.php with argument "file". For Documentation see http://s3db.org/documentation/api-functions</font></form></td></tr><br><br>
